...
def main():
    ...
    meal_sites.reset_index(drop=True, inplace=True)
    youth_sites = pd.read_csv(
        meals_path + 'Youth_Meal_Sites_manager_All.csv')
    youth_sites.reset_index(drop=True, inplace=True)
    ...
    all_sites_meals.drop([1056, 3063], axis=0, inplace=True) #artifact of the scraping, remove

    pdb.set_trace()
    all_sites_meals.report_date

    visits_path = '../../stats/food_sites_time_series/'
    all_sites_veraset = pd.read_csv(visits_path+'food_sites_time_series.csv')

    mask = [x in all_sites_meals.site_name.unique()
            for x in all_sites_veraset.name.unique()]
    pdb.set_trace()
